	      					...
	      					<tbody>
	      						@forelse($subjects as $subject)
	      							<tr>
	      								...
	      								<td>
	      									@if($subject->hide == 1)
	      										<button class="btn btn-xs btn-primary" onclick="unhideMe({{ $subject->id }})">Tampilkan</button>
	      									@else
	      										<button class="btn btn-xs btn-danger" onclick="hideMe({{ $subject->id }})">Sembunyikan</button>
	      									@endif
	      								</td>
	      								<td>
...
<script type="text/javascript">
	...
	function hideMe(id) {
		...
		if(confirm("Sembunyikan Bidang Studi ini di Halaman Ujian?!")) {

			$.ajax({
				url 		: "{{ URL::to('subject') }}/"+id+"/hide",
				type 		: "PUT",
				success 	: function() {
	...
	function unhideMe(id) {

		if(confirm("Tampilkan Bidang Studi ini di Halaman Ujian?!")) {

			$.ajax({
				url 		: "{{ URL::to('subject') }}/"+id+"/unhide",
				type 		: "PUT",
				success 	: function() {
